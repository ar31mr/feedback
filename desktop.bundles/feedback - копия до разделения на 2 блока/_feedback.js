...
;
/* ../../desktop.blocks/feedback/feedback.js begin */
modules.define(
    'feedback',
    ['i-bem__dom','jquery'],
...
provide(BEMDOM.decl(this.name, {
    onSetMod : {
            'js': {
                'inited': function() {                  	 
                	 ...
                	 this._ajax_answer = this.findElem('ajax-answer');
                	 this.setMod(this._ajax_answer, 'statys', 'noshow');
                } 
            },
            'statys':{
            	'send': function(){
        ...
        //Ф-ция проверки формы перед отправкой на сервер
        //НЕ ИСПОЛЬЗУЮ! ПРОВЕРКА через API
        _checkForm: function () {
        	alert('Проверяем форму и делаем невидимой и неактивный button');
        	this._statusForm('invisible');

            var checkForm_error='';

        	if(this._feedback_username.getVal()==''){
            	checkForm_error+='Имя не может быть пустым!\n';            	
                //Установим фокус и сделаем бордер
            	$('.feedback__feedback-username span')
            		.focus()
            		.css('border', '1px solid red');
            }            
            //Если есть ошибки
            if(checkForm_error){
                alert(checkForm_error);
            }
            //
            else{                
                //Отправка формы
            ...
            this._statusForm('visible');
        },

        /* visible invisible */
		_statusForm: function (status) {
        	//alert(status);
        	
        	if(status == 'invisible'){
        		$('.feedback').css('opacity', '0.7');            			
            	$('.feedback__f-button')
    ...
    },
    {
        ...
        //this._checkForm();                
            	...
            	//Разобраться с видимостью блока ajax-answer
            	//this.setMod(this._ajax_answer, 'statys', 'show');            	

            	//Меняем статус на время выполнения запроса ... ajax ...
	           	//this.setMod('statys', 'send'); 
                ...
                //alert(formData[0].name + ' ' +formData[0]['name']);               
                ...
                $.ajax({
                  ...
                  success: function(answer){                       
                    $('.feedback__ajax-answer').fadeIn('slow');
                    $('.content').append(answer);
                    $('.feedback__ajax-answer').html(answer.status);
                    console.log(answer);
                  }
                });
            	
                /*
            	this._ajax_answer.html(
            		'<br>Данные для отправки ... Повешать валидацию и т.п. перед отправкой ajax '+
            		'<br>Значение _feedback_username: ' + this._feedback_username.getVal() +
            		'<br>Значение _feedback_phone: ' + this._feedback_phone.getVal() +
            		'<br>Значение _feedback_email: ' + this._feedback_email.getVal() +
            		'<br>Значение _feedback_message: ' + this._feedback_message.getVal()
            	);*/
            });       
        }
...
/* ../../desktop.blocks/feedback/feedback.js end */
;
...
;
/* ../../libs/bem-components/common.blocks/textarea/textarea.js begin */
/**
 * @module textarea
 */
...
/* ../../libs/bem-components/common.blocks/textarea/textarea.js end */
;
